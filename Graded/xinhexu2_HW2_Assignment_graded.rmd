...
See comments below for feedback. I may have numbers associated with my feedback. The higher the number the more signifciant the feedback.
```
...
```{remark class.source="tacomments"}
[Efficiency 1] Indirect approach versus using ORDER. e.g. dat[-order(dat$x)[1:100],]
```
...
```{remark class.source="tacomments"}
[Efficiency 0] It's even better to use zlist here
```

```{remark class.source="tacomments"}
[Beauty 1] For readability, it's better to use df[rows, cols] versus df$cols[rows]
```
...
```{remark class.source="tacomments"}
[Correctness 1] Autocorrelation is about correlation between observations not the columns
```
...
```{remark class.source="tacomments"}
[Beauty 0] Did you know that 'letters' are a preprogrammed variable?
```
...
```{remark class.source="tacomments"}
[Beauty 0]
* More readable as ccso_data$CITY  c("CHAMPAIGN","URBANA").
```
...
```{remark class.source="tacomments"}
[Beauty 0] Having to put quotes around all of your variables is a pain. It's more elegant to rename the columns to remove the spaces. e.g. colnames(ccso_data2) <- gsub(" ",".", colnames(ccso_data2))
```
...
```{remark class.source="tacomments"}
[Beauty 3] One of the good thinkgs about data.table is you can drop the newdata$
```
